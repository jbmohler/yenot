...
class WriteChunk:
    ...
    def upsert_rows(self, tname, table):
        ...
        for row in cols:
            if row.column_name in tosave:
                if row.data_type in ('character', 'character varying', 'text'):
                    ...
                    coltypes[row.column_name] = 'numeric({}, {})'.format(row.numeric_precision, row.numeric_scale)
                elif row.data_type in ('date', 'boolean', 'json', 'integer', 'smallint', 'uuid'):
                    # bit of a catch-all
        ...
        mog.primary_key = sqlread.sql_1row(self.conn, PRIM_KEY_SELECT, {'sname': sx, 'tname': tx})
        mog.table = tname
...
def _mogrify_values(cursor, rows, row2dict, columns, types):
    ...
    qualnames = ['%({})s::{}'.format(cname, t) if t != None else '%({})s'.format(cname) for cname, t in zip(columns, types)]
...
def mogrify_values_anon(cursor, rows, columns, types=None):
    return _mogrify_values(cursor, rows, lambda r: dict(zip(columns, r)), columns, types)
...
/*NAME*/(/*COLUMNS*/) as (
    values/*REPRESENTED*/
        ...
        with conn.cursor() as cursor:
            mogrifications = mogrify_values(cursor, table.rows, columns, self.column_types)

        return result_template \
                .replace("/*REPRESENTED*/", mogrifications) \
...
from staging
where {pkm}""".format(**interpolations)

        delete = """
with staging({pknames}) as (
    values/*REPRESENTED*/
)
delete from {fqtn} where ({pknames}) in (select * from staging)""".format(pknames=','.join(pkey), **interpolations)
        ...
        with conn.cursor() as cursor:
            ...
            if len(table.deleted_keys) > 0:
                mogrifications = mogrify_values_anon(cursor, table.deleted_keys, pkey, {})
            ...
            if len(pkey) == 1:
                ...
                rows2 = []
            if len(rows1) > 0:
                mogrifications = mogrify_values(cursor, rows1, collist, self.column_types)
            ...
            if len(rows2) > 0:
                mogrifications = mogrify_values(cursor, rows2, collist, self.column_types)
                # insert
