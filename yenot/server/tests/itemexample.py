...
where id=%(item)s"""
    ...
    with app.dbconn() as conn:
        ...
        columns = api.tab2_columns_transform(rawdata[0], insert=[('price', 'revenue_level')])
...
def put_api_test_item_record(itemid):
    item = api.table_from_tab2('item', required=['name', 'price'], amendments=['id'], allow_extra=True)
...
def get_api_test_items_list():
    ...
    with app.dbconn() as conn:
        ...
        results.tables['items', True] = api.sql_tab2(conn, "select * from item", column_map=cm)
    return results.json_out()
